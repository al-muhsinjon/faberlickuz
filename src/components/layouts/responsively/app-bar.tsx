...
const AppBar = ({ locale }: { locale: string }) => {
  ...
  return (
    <>
      <div className="flex px-4 py-3 bg-white items-center border-b lg:hidden md:hidden justify-between shadow-md">
        <Button
          onClick={() => {
            isOpen ? onClose() : onOpen();
          }}
          className={`border px-4 py-2 rounded-lg ${
            isOpen
              ? "bg-main text-white hover:bg-main/90 ease-linear"
              : "border-main text-main font-semibold hover:text-main/80"
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {searchOpen.isOpen ? <X size={20} /> : <Search size={20} />}
          </Button>
          <Link href={`/${locale}/about`}>
            <Button
              className="border px-4 py-2 rounded-lg border-main text-main font-semibold hover:bg-main hover:text-white flex items-center gap-2"
            >
              <Contact size={20} />
              <span className="hidden lg:block ml-2">Contact</span>
            </Button>
          </Link>
        </div>
      </div>
      {searchOpen.isOpen && (
        <>
          <div className="w-full px-6 flex mt-2">
            <Input
              type="search"
              placeholder="Search..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full px-4 py-2 rounded-md outline-none bg-mainGray/15"
            />
          </div>
          {opened && (
            <div className="w-full mx-auto left-0 absolute z-10 overflow-y-auto bg-white shadow-lg max-h-60 mt-2 rounded-md">
              {product && product.results && product.results.length > 0 ? (
                product.results.map((prod) => (
                  <Link
